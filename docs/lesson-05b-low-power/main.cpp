...
// Sleep tuning.
#define IDLE_SLEEP_MS   400          // sleep after this long with no touch
#define SLEEP_TIMER_US  1000000ULL   // also wake every 1 s to refresh the gauge
...
void loop() {
  ...
  // Sleep only when: on battery, idle a while, and no animation is mid-flight.
  bool idle = (millis() - lastActivityMs) > IDLE_SLEEP_MS;
  if (!onUsbCached && idle && lv_anim_count_running() == 0) {
    enterLightSleep();
  } else {
    delay(5);
  }
