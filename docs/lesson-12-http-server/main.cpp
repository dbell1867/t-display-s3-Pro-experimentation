...
void loop() {
  ...
  // GPIO 12 button (falling edge) brings up the WiFi hotspot. Physical button, not
  // touch, so it works the same way the viewfinder's exit does.
  static bool wifiBtnPrevHigh = true;
  bool wifiHigh = (digitalRead(BTN_WIFI) == HIGH);
  if (wifiBtnPrevHigh && !wifiHigh) { wifiBtnPrevHigh = wifiHigh; runWifiAP(); return; }
