...
// ---- Stage 10: WiFi first light — scan for networks ------------------------
// The radio is independent of every bus we've fought (SPI / I2C / DVP): it needs
// only power and RAM, so there's no sharing dance. A SCAN is the radio's "hello
// world" — it proves the antenna works, and needs NO credentials and NO STA-vs-AP
// decision (both of those come when we actually JOIN a network). Triggered by the
// GPIO 12 button — the rocker half we re-discovered on 2026-07-21, now earning a
// job. Draws straight to the panel (touch still works, no camera); tap to exit.
static void runWifiScan(void) {
  setBacklight(BL_MAX_DUTY);
  panel->fillScreen(RGB565_BLACK);
  panel->setTextSize(2);
  panel->setTextColor(RGB565_WHITE);
  panel->setCursor(4, 8);
  panel->print("WiFi scan...");

  WiFi.mode(WIFI_STA);     // station mode: we look FOR access points (not being one)
  WiFi.disconnect();        // drop any stale association so the scan is clean
  delay(100);
  if (Serial) Serial.println("\n--- WiFi scan ---");
  int n = WiFi.scanNetworks();   // BLOCKS ~2-4 s; returns the count (negative = error)

  panel->fillRect(0, 8, SCREEN_W, 24, RGB565_BLACK);
  panel->setTextSize(2);
  panel->setTextColor(RGB565_GREEN);
  panel->setCursor(4, 8);
  char hdr[24];
  snprintf(hdr, sizeof(hdr), "%d networks", n < 0 ? 0 : n);
  panel->print(hdr);

  // One row per network: SSID, signal (RSSI, dBm — closer to 0 is stronger), and a
  // '#' if it's encrypted. Grey = locked, white = open. Cap the list to what fits.
  panel->setTextSize(1);
  int shown = (n < 0) ? 0 : (n > 22 ? 22 : n);
  for (int i = 0; i < shown; i++) {
    bool open = (WiFi.encryptionType(i) == WIFI_AUTH_OPEN);
    char line[48];
    snprintf(line, sizeof(line), "%-20.20s %4d%s",
             WiFi.SSID(i).c_str(), WiFi.RSSI(i), open ? "" : " #");
    panel->setTextColor(open ? RGB565_WHITE : 0xC618 /* silver */);
    panel->setCursor(4, 40 + i * 12);
    panel->print(line);
    if (Serial) Serial.printf("  %-24s %4d dBm  %s\n",
                              WiFi.SSID(i).c_str(), WiFi.RSSI(i), open ? "open" : "locked");
  }

  panel->setTextColor(RGB565_WHITE);
  panel->setCursor(4, SCREEN_H - 14);
  panel->print("tap to exit");

  WiFi.scanDelete();       // free the scan result list
  WiFi.mode(WIFI_OFF);     // radio OFF again: idling it burns ~tens of mA (Stage 5 work)

  // Wait for a screen tap, then hand the panel back to LVGL. prevTouch starts true
  // so a finger left on the screen can't instantly dismiss the results.
  bool prevTouch = true;
  while (true) {
    int16_t tx[1], ty[1];
...
void loop() {
  ...
  // GPIO 12 button (falling edge) runs a WiFi scan. Physical button, not touch,
  // so it works the same way the viewfinder's exit does.
