...
static void cameraDriverProbe(void) {
  ...
  // The backlight already took an LEDC channel via ledcAttach() (the Arduino 3.x
  // core auto-allocates, almost certainly channel 0). Point the camera somewhere
  // else so initialising it can't reprogram the timer driving the screen.
  ...
  // The camera driver installed its own SCCB master on pins 5/6 — the bus touch,
  // the PMU and the ALS live on. esp_camera uses I2C PORT 1; Arduino Wire uses
  // PORT 0. Same pins, different peripheral, muxed through the GPIO matrix. After
  // esp_camera_deinit() the pins still point at the (now-dead) port-1 driver, so
  // every read fails. A bare Wire.begin() early-returns when Wire is already up
  // and never re-does the pin mux — which is why the first recovery attempt froze
  // the gauge and the second broke PPM.init() outright. Tear Wire fully DOWN so
  // the next begin() re-mux'es pins 5/6 back to port 0, then re-establish the
  // three drivers' register config on top.
  Wire.end();
  delay(10);
