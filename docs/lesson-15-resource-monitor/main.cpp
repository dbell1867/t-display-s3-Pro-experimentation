...
#include <BLEDevice.h>           // Bluetooth LE (Stage 13); bundled, BLE-only on the S3
#include <BLEServer.h>
#include <BLE2902.h>             // CCCD descriptor: lets a client enable notifications

// Every pin and tuning constant now lives in include/board_pins.h as typed
// `constexpr` values instead of preprocessor #defines (Stage 15 refactor).
#include "board_pins.h"
#include "ResourceMonitor.h"

// The resource monitor: heap/PSRAM/CPU/task/SD instrumentation (Stage 15).
...
// A status line UNDER the viewfinder image (frames only cover the image rect, so
// text below oy+fh survives until the next capture rewrites it). Direct panel
// text — we're not in LVGL while streaming.
...
// ---- Stage 9: capture the current frame to a JPEG on the SD card ------------
// Grab a fresh frame, encode RGB565 -> JPEG with the bundled converter, and write
// it under the next free /IMG_NNNN.JPG. No bus conflict: the camera data path is
// the DVP parallel bus, encoding is pure CPU, and the SD write is SPI (shared with
// the display but bracketed per transfer, Stage 6). imgSeq is a static counter —
// on the first capture after a reboot it scans up past any files already on the
// card, then just increments.
static bool captureStill(int msgY) {
  static uint16_t imgSeq = 1;
  ...
  uint8_t *jpg = NULL;
  size_t   jpgLen = 0;
  bool ok = frame2jpg(fb, 80, &jpg, &jpgLen);   // quality 80; frame2jpg malloc's `jpg`
  esp_camera_fb_return(fb);                       // done with the raw frame immediately
  if (!ok) { drawViewfinderMsg(msgY, "JPEG encode failed", RGB565_RED); return false; }

  char path[24];
  while (true) {                                  // find the next free filename
    snprintf(path, sizeof(path), "/IMG_%04u.JPG", imgSeq);
    if (!SD.exists(path)) break;
    imgSeq++;
  }

  File f = SD.open(path, FILE_WRITE);             // path is known-free -> creates it
  bool wrote = false;
  if (f) { wrote = (f.write(jpg, jpgLen) == jpgLen); f.close(); }
  free(jpg);                                       // OUR job: frame2jpg allocated it

  if (Serial) Serial.printf("capture %s: %s (%u bytes)\n",
                            path, wrote ? "OK" : "WRITE FAIL", (unsigned)jpgLen);
  if (wrote) { imgSeq++; drawViewfinderMsg(msgY, path, RGB565_GREEN); }
  else       { drawViewfinderMsg(msgY, "SD write failed", RGB565_RED); }
  return wrote;
}

// ---- Stage 8/9: live viewfinder --------------------------------------------
...
// and the gauge are frozen for the duration. So the ONE physical button (GPIO 16,
// the only input that survives the takeover, Stage 5f) does both jobs by DURATION:
//   short tap   -> capture a still to SD (Stage 9)
//   long hold   -> exit back to the gauge
//
...
static void runViewfinder(void) {
  ...
  const int msgY = oy + fh + 6;          // status line just below the image
  drawViewfinderMsg(msgY, "tap = save   hold = exit", RGB565_WHITE);

  // Seed the edge detector with the button's CURRENT level, so a still-held button
  // from entering can't instantly trip an action.
  const uint32_t LONGPRESS_MS = 700;     // hold this long = exit; shorter = capture
  ...
  while (!exiting) {
    ...
    // One button, two actions by DURATION. Measure the press on RELEASE for a
    // short tap (capture); trip the exit the moment a hold crosses the threshold
    // (so you don't have to release to leave).
    bool high = (digitalRead(BTN_WAKE) == HIGH);
    if (prevHigh && !high) {                             // press begins
      pressStart = millis();
    } else if (!prevHigh && high) {                      // release
      uint32_t held = millis() - pressStart;
      if (held >= 40 && held < LONGPRESS_MS) captureStill(msgY);   // 40 ms debounces bounce
    } else if (!high && (millis() - pressStart) >= LONGPRESS_MS) {
      exiting = true;                                    // long hold: leave
    }
    prevHigh = high;
  }
...
// ---- Stage 11: WiFi connect — the board becomes an ACCESS POINT ------------
// AP mode = the board IS the network: a phone joins it directly, no router, no
// stored home credentials. An access point's OWN SSID/password are meant to be
// SHARED (we show them on screen), so hard-coding them is correct — unlike a home
// WiFi password, they're not a secret to keep out of git. WPA2 needs a password of
// >= 8 characters. This step just proves ASSOCIATION: watch the live client count
// go 0 -> 1 when your phone joins. Serving the photos over it is Stage 12.
// Triggered by GPIO 12; draws to the panel (touch works, no camera); tap to stop.
...
static void runWifiAP(void) {
  ...
  if (Serial) Serial.printf("\n--- WiFi AP %s: SSID=%s IP=%s ---\n",
                            ok ? "up" : "FAILED", AP_SSID, ip.toString().c_str());

  panel->setTextSize(2);
  panel->setTextColor(ok ? RGB565_GREEN : RGB565_RED);
  panel->setCursor(4, 8);
  panel->print(ok ? "WiFi Hotspot" : "AP FAILED");
  ...
  panel->setCursor(4, 78);   panel->print(AP_SSID);
  ...
  while (true) {
    ...
    if (clients != lastCount) {
      ...
      panel->setTextSize(2);
      panel->setTextColor(clients > 0 ? RGB565_GREEN : 0xC618 /* silver */);
      panel->setCursor(4, countY);
      char cline[24];
      snprintf(cline, sizeof(cline), "Clients: %d", clients);
      panel->print(cline);
      if (Serial) Serial.printf("AP clients: %d\n", clients);
      lastCount = clients;
    }
    int16_t tx[1], ty[1];
...
// ---- Stage 13: Bluetooth LE — the other half of the radio ------------------
// The ESP32-S3 is Bluetooth LOW ENERGY only (no Classic, so no SPP serial). BLE
// works by exposing SERVICES made of CHARACTERISTICS that a client reads or
// subscribes to. We expose the STANDARD Battery Service (0x180F) with the Battery
// Level characteristic (0x2A19), fed by the SY6970 gauge from Stage 5a — so any
// generic BLE app (nRF Connect, LightBlue) recognises it and can read/notify the %.
// Advertising IS the BLE "first light"; the service gives the phone something real
// to read. Entered by HOLDING GPIO 12 (a tap is WiFi).
static volatile bool bleConnected = false;

// Track connect/disconnect. On disconnect, restart advertising so the next phone
// can find us again (BLE stops advertising while a client is connected).
...
static void runBleService(void) {
  setBacklight(BL_MAX_DUTY);
  panel->fillScreen(RGB565_BLACK);
  panel->setTextSize(2); panel->setTextColor(RGB565_GREEN);
  panel->setCursor(4, 8);   panel->print("Bluetooth LE");
  panel->setTextSize(1); panel->setTextColor(RGB565_WHITE);
  panel->setCursor(4, 40);  panel->print("Connect with a BLE app");
  panel->setCursor(4, 52);  panel->print("(e.g. nRF Connect):");
  panel->setTextSize(2);
  panel->setCursor(4, 72);  panel->print("TDisplay-S3-Pro");
  panel->setTextSize(1);
  panel->setCursor(4, 100); panel->print("Battery Service 0x180F");

  bleConnected = false;
  BLEDevice::init("TDisplay-S3-Pro");
  BLEServer *srv = BLEDevice::createServer();
  srv->setCallbacks(new BleConnCallbacks());
  ...
  BLE2902 *cccd = new BLE2902();     // the CCCD: the client writes here to subscribe
  chr->addDescriptor(cccd);          // cccd->getNotifications() tells us if it did
  svc->start();
  BLEAdvertising *adv = BLEDevice::getAdvertising();
  adv->addServiceUUID(BLEUUID((uint16_t)0x180F));
  adv->setScanResponse(true);
  BLEDevice::startAdvertising();
  if (Serial) Serial.println("\n--- BLE advertising: TDisplay-S3-Pro (Battery Service) ---");

  panel->setTextColor(RGB565_WHITE);
  panel->setCursor(4, SCREEN_H - 14);
  panel->print("tap to stop Bluetooth");

  const int statusY = 130, pctY = 158;
  int  lastPct = -1, lastState = -1;
  bool prevTouch = true;
  uint32_t lastUpd = 0;
  while (true) {
    if (millis() - lastUpd >= 1000) {                 // refresh the battery once a second
      lastUpd = millis();
      uint16_t mv  = pmuOk ? PPM.getBattVoltage() : 0;
      int      pct = battPercent(mv);
      uint8_t  lvl = (uint8_t)constrain(pct, 0, 100);
      bool     sub = cccd->getNotifications();        // did the client enable notify?
      chr->setValue(&lvl, 1);
      if (bleConnected) {
        chr->notify();                                // push to a subscribed client
        if (Serial) Serial.printf("BLE notify %d%% (subscribed=%d)\n", pct, sub ? 1 : 0);
      }

      if (pct != lastPct) {
        panel->fillRect(0, pctY, SCREEN_W, 22, RGB565_BLACK);
        panel->setTextSize(2); panel->setTextColor(RGB565_GREEN);
        panel->setCursor(4, pctY);
        char pl[24]; snprintf(pl, sizeof(pl), "Battery: %d%%", pct);
        panel->print(pl);
        lastPct = pct;
      }

      // Show connection AND subscription state — so it's obvious whether the app
      // has enabled notifications (the thing that was hard to find in the UI).
      int state = (bleConnected ? 2 : 0) | (sub ? 1 : 0);
      if (state != lastState) {
        const char *msg; uint16_t col;
        if (!bleConnected)  { msg = "Advertising...";            col = 0xC618; }
        else if (!sub)      { msg = "Connected - enable Notify"; col = RGB565_WHITE; }
        else                { msg = "Connected + Notifying";     col = RGB565_GREEN; }
        panel->fillRect(0, statusY, SCREEN_W, 18, RGB565_BLACK);
        panel->setTextSize(1); panel->setTextColor(col);
        panel->setCursor(4, statusY); panel->print(msg);
        lastState = state;
        if (Serial) Serial.printf("BLE state: %s\n", msg);
      }
    }
    int16_t tx[1], ty[1];
    bool touched = (touch.getPoint(tx, ty, 1) > 0);
  ...
  BLEDevice::deinit(true);   // stop advertising, disconnect any client, free the controller
...
// ---- Stage 14: radio power bench -------------------------------------------
// The radios are the hungriest thing on this board (Derek noticed on the meter).
// This holds each radio STATE steady — at constant full brightness, charging
// disabled — so the inline USB meter can settle, exactly like the Stage 5e sleep
// bench. The DIFFERENCES between states isolate each radio's cost; the screen and
// CPU are common-mode and cancel out. GPIO 16 = next state, tap = exit. The key
// comparison is WiFi-idle vs BLE-idle: an AP has no modem sleep, BLE barely sips.
//
// Returns true to advance, false if the user tapped to exit.
static bool radioHold(const char *title, const char *l1, const char *l2) {
  panel->fillScreen(RGB565_BLACK);
  panel->setTextSize(2); panel->setTextColor(RGB565_GREEN);
  panel->setCursor(4, 8);  panel->print(title);
  panel->setTextSize(1); panel->setTextColor(RGB565_WHITE);
  if (l1) { panel->setCursor(4, 48); panel->print(l1); }
  if (l2) { panel->setCursor(4, 64); panel->print(l2); }
  panel->setTextColor(0xC618 /* silver */);
  panel->setCursor(4, SCREEN_H - 30); panel->print("read the inline meter now");
  panel->setCursor(4, SCREEN_H - 14); panel->print("GPIO16 = next   tap = exit");
  if (Serial) Serial.printf(">>> RADIO BENCH: %s\n", title);

  bool btnPrev = (digitalRead(BTN_WAKE) == HIGH);
  bool touchPrev = true;
  while (true) {
    bool btn = (digitalRead(BTN_WAKE) == HIGH);
    if (btnPrev && !btn) return true;              // GPIO 16 press -> next state
    btnPrev = btn;
    int16_t tx[1], ty[1];
    bool touched = (touch.getPoint(tx, ty, 1) > 0);
    if (touched && !touchPrev) return false;       // tap -> exit
    touchPrev = touched;
    delay(20);
  }
}

static void runRadioBench(void) {
  setBacklight(BL_MAX_DUTY);
  if (pmuOk) PPM.disableCharge();   // charge current would contaminate readings (Stage 5e)
  WiFi.mode(WIFI_OFF);

  bool go = radioHold("0 Radios OFF", "baseline: WiFi + BLE off", "note this reading");

  if (go) {                          // ---- WiFi states ----
    WiFi.mode(WIFI_AP);
    WiFi.softAP(AP_SSID, AP_PASS);
    go = radioHold("1 WiFi AP idle", "advertising, no client", "(an AP has no modemsleep)");
    if (go) go = radioHold("2 WiFi + client", "JOIN TDisplay-S3-Pro", "from your phone now");
  ...
  if (go) {                          // ---- BLE states ----
    BLEDevice::init("TDisplay-S3-Pro");
    BLEServer *srv = BLEDevice::createServer();
    BLEService *svc = srv->createService(BLEUUID((uint16_t)0x180F));
    BLECharacteristic *chr = svc->createCharacteristic(
        BLEUUID((uint16_t)0x2A19),
        BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_NOTIFY);
    chr->addDescriptor(new BLE2902());
    uint8_t lvl = 50; chr->setValue(&lvl, 1);
    svc->start();
    BLEDevice::getAdvertising()->addServiceUUID(BLEUUID((uint16_t)0x180F));
    BLEDevice::startAdvertising();
    go = radioHold("3 BLE advertising", "not connected", "(compare to WiFi idle!)");
    if (go) radioHold("4 BLE connected", "connect + subscribe", "in nRF Connect");
    BLEDevice::deinit(true);
  }

  if (pmuOk) PPM.enableCharge();
...
void setup() {
  ...
  // Stage 15: baseline the memory BEFORE the drivers allocate anything, so the
  // report at the end of setup() shows what bring-up actually cost.
  resMon.begin();
  ...
  pinMode(BTN_WIFI, INPUT_PULLUP);   // GPIO 12 rocker half: triggers the WiFi scan
  ...
  // Stage 15: the full picture, once everything is up. The DELTA section shows
  // what the whole bring-up (display, LVGL, SD, camera probe) cost in RAM.
  resMon.reportAll(Serial);
}
...
void loop() {
  ...
  // GPIO 12 does BOTH radios, split by press DURATION (the Stage 9 tap/hold trick):
  //   TAP  (< 700 ms) -> WiFi hotspot
  //   HOLD (>= 700 ms) -> Bluetooth LE
  // The hold fires the moment it crosses the threshold (no need to release); a tap
  // is decided on release. g12Down is consumed (set to 0) once a gesture fires so it
  // can't double-trigger.
  static bool     g12Prev = true;
  static uint32_t g12Down = 0;
  bool g12 = (digitalRead(BTN_WIFI) == HIGH);
  if (g12Prev && !g12) {                                           // press begins
    g12Down = millis();
  } else if (!g12 && g12Down && (millis() - g12Down) >= 700) {     // held past threshold
    // Stage 15: bracket the radio so we can see what it COST in RAM. Stage 14
    // measured the radios in milliamps; this is the same question in bytes.
    g12Down = 0; g12Prev = g12;
    resMon.mark("before BLE");
    runBleService();
    resMon.reportDelta(Serial);
    return;                                                       // HOLD -> BLE
  } else if (!g12Prev && g12) {                                    // released
    uint32_t held = millis() - g12Down; g12Down = 0;
    if (held >= 40 && held < 700) {                                        // TAP -> WiFi
      g12Prev = g12;
      resMon.mark("before WiFi AP");
      runWifiAP();
      resMon.reportDelta(Serial);
      return;
    }
  ...
  // GPIO 16 (the OTHER rocker half) at top level launches the RADIO POWER BENCH
  // (Stage 14). g16Init seeds the level on the first pass so a button still held
  // from a deep-sleep wake can't fire a phantom falling edge.
  static bool g16Prev = true, g16Init = false;
  bool g16 = (digitalRead(BTN_WAKE) == HIGH);
  if (!g16Init)              { g16Prev = g16; g16Init = true; }
  else if (g16Prev && !g16)  { g16Prev = g16; runRadioBench(); return; }
  g16Prev = g16;
  ...
  // Stage 15: a compact resource line every 10 s. Serial-only on purpose — a
  // monitor that redraws the screen would perturb the very thing it measures.
  static uint32_t lastRes = 0;
  if (millis() - lastRes >= 10000) {
    resMon.line(Serial);
    lastRes = millis();
  }
