...
static void initSD(void) {
  ...
  if (sdOk) {
    ...
    const char *t = (type == CARD_MMC) ? "MMC" : (type == CARD_SD)  ? "SD"
                  : (type == CARD_SDHC) ? "SDHC" : "?";
    // Compact: the boot line is ~30 chars wide at font 14 on a 222 px screen.
    snprintf(sdInfo, sizeof(sdInfo), "SD%lluG@%uM",
             SD.cardSize() / (1024ULL * 1024ULL * 1024ULL), (unsigned)(sdHz / 1000000));
  } else {
...
void setup() {
  ...
  if (sdOk) {
    ...
    char tail[16];
    snprintf(tail, sizeof(tail), " %s L%lu", st ? "rw" : "RWFAIL", (unsigned long)boots);
    strncat(sdInfo, tail, sizeof(sdInfo) - strlen(sdInfo) - 1);
  }
  lv_label_set_text_fmt(bootLabel, "#%u %s %s", bootCount, wakeReason, sdInfo);
